...
describe('versions', () => {
  it('exposes the latest version as 0.1.0 (matches hanflow source)', () => {
    expect(LATEST_VERSION).toBe('0.1.0');
    expect(VERSIONS[VERSIONS.length - 1]).toBe(LATEST_VERSION);
  });

  it('detects known version strings', () => {
    expect(isKnownVersion('0.1.0')).toBe(true);
  ...
  it('strips a version prefix if present, else returns slug unchanged', () => {
    expect(stripVersionPrefix(['0.1.0', 'quick-start'])).toEqual(['quick-start']);
    expect(stripVersionPrefix(['quick-start'])).toEqual(['quick-start']);
  });

  it('builds a versioned path with no prefix for latest', () => {
    expect(versionedPath('quick-start', '0.1.0')).toBe('quick-start');
  });
